...
export const useWebSocket = (
  ...
  const intentionallyClosedSockets = useRef(new WeakSet<WebSocket>());
  ...
  const initConnection = useCallback(() => {
    ...
    if (ws.current) {
      intentionallyClosedSockets.current.add(ws.current);
      ws.current.close();
      ws.current = null;
    }
    ...
    try {
      ...
      socket.onclose = (event) => {
        const wasIntentional = intentionallyClosedSockets.current.has(socket);
        if (!wasIntentional) {
