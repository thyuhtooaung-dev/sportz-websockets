...
async function readJsonFile(fileUrl: URL) {
...
async function insertCommentary(matchId: number, entry: any) {
  const payload: any = {
...
function extractRuns(entry: any) {
...
function scoreDeltaFromEntry(entry: any, match: any) {
...
function cricketScoreDelta(entry: any, match: any) {
...
function replaceTrailingTeam(message: any, replacements: Map<string, string>) {
...
function expandFeedForMatches(feed: any[], seedMatches: any[]) {
  ...
  const templateBySport = new Map<string, any>();
...
function buildRandomizedFeed(feed: any[], matchMap: Map<number, any>) {
  const buckets = new Map<number | null, any[]>();
  for (const entry of feed) {
  ...
  for (const [matchId, entries] of buckets) {
    if (!Number.isInteger(matchId) || matchId === null) {
      continue;
    }
...
function getMatchEntry(entry: any, matchMap: Map<number, any>) {
  if (!Number.isInteger(entry.matchId)) {
    return null;
  }
